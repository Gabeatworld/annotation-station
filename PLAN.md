...
- **Permissions and bundles.** TCC keys grants by bundle id + code signature. Ad-hoc sign (`codesign --force --deep --sign - AnnotationStation.app`) so the signature is stable across rebuilds; otherwise macOS re-prompts every build. Run the `.app`, never the bare binary, when testing permissions.
